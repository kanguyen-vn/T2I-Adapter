...
parser.add_argument("--num_workers", type=int, default=0, help="the prompt to render")
parser.add_argument(
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            print(f"{model.module.device = }")
            print(f"{model.device = }")
            print(f"{model.module.logvar.device = }")
            ...
            if (rank == 0) and (
                (current_iter + 1) % config["training"]["save_freq"] == 0
            ):
        ...
        if rank == 0:
            for data in val_dataloader:
                with torch.no_grad():
                    ...
                    cv2.imwrite(
                        os.path.join(
                            experiments_root, "visualization", "mask_%04d.png" % epoch
                        ),
                        im_mask,
                    )
                    features_adapter = model_ad(mask)
